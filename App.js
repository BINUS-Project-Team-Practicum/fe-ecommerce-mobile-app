...
export default function App() {
  ...
  return <SafeAreaView style={{ flex: 1, backgroundColor: '#F8FAFC' }}><StatusBar barStyle="dark-content" />
    ...
    {stage === 'app' && <AppShell user={user} products={demoProducts} cart={cart} wishlist={wishlist} onLogin={() => setStage('auth')} onAddToCart={addToCart} onUpdateQuantity={updateQuantity} onToggleWishlist={(id) => setWishlist((items) => items.includes(id) ? items.filter((item) => item !== id) : [...items, id])} />}
  </SafeAreaView>;
}
